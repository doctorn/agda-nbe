...
Λ′ : ∀ {Γ A B} → Γ × A Gl.⇒ (⊤ × B) → CommaObj.α Γ Psh.⇒ 𝓡₀ (A ⇒ B)
Λ′ {Γ} {A} {B} f = Pullback.universal (ψ {A} {B} Psh.⊗ ϕ {A} {B}) {h₁ = h₁} {h₂ = h₂} coherence
  where module Γf = NaturalTransformation (CommaObj.f Γ)
        module Γα = Functor (CommaObj.α Γ)

        commute : ∀ {Δ Ξ} (g : 𝒲 Ξ Δ) {γ₁ γ₂} (γ₁≈γ₂ : Setoid._≈_ (Γα.₀ Δ) γ₁ γ₂)
                  → ! ∷ Λ (𝒵 (Comma⇒.h f ∘ (π (Γf.η Ξ ⟨$⟩ (Γα.₁ g ⟨$⟩ γ₁)) ∷ 𝓏)))
                    S.≈ ! ∷ 𝓏 [ ! ∷ Λ (𝒵 (Comma⇒.h f ∘ (π (Γf.η Δ ⟨$⟩ γ₂) ∷ 𝓏))) [ W.₁ g ] ]
        commute {Δ} {Ξ} g {γ₁} {γ₂} γ₁≈γ₂ = ∷-congᵣ (begin
            Λ (𝒵 (Comma⇒.h f ∘ (π (Γf.η Ξ ⟨$⟩ (Γα.₁ g ⟨$⟩ γ₁)) ∷ 𝓏)))
        ...
        coherence {Δ} {γ₁} {γ₂} γ₁≈γ₂ {Ξ} {y₁ , θ₁} {y₂ , θ₂} (y₁≈y₂ , θ₁≈θ₂) = begin
          ...
          ≈⟨ ∷-congᵣ (app-congₗ (𝒵-cong (S.sym (commute θ₁ (Setoid.sym (Γα.₀ Δ) γ₁≈γ₂))))) ⟩
