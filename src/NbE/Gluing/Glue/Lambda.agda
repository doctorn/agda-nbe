...
module NbE.Gluing.Glue.Lambda {a} (𝒰 : Set a) where
...
open import NbE.Gluing.Glue.Cartesian 𝒰 using (CC; ⊤; _×_)
open import NbE.Gluing.Glue.Yoga 𝒰 using (𝓡₀; ϕ; ψ; ↓₀; ↑₀; yoga₀)
...
module _ {Γ} {A} {B} (f : Γ × A Gl.⇒ (⊤ × B)) where
  ...
  coherence {Δ} {γ₁} {γ₂} γ₁≈γ₂ {Ξ} {y₁ , θ₁} {y₂ , θ₂} (y₁≈y₂ , θ₁≈θ₂) = begin
      ! ∷ (𝓏 [ ! ∷ (Λ (𝒵 (Comma⇒.h f ∘ (π (Γf.η Δ ⟨$⟩ γ₁) ∷ 𝓏))) [ E.₁ θ₁ ]) ]) ⦅ 𝒵 (𝔦₀.η A Ξ ⟨$⟩ (↓₀.η A Ξ ⟨$⟩ y₁)) ⦆
    ...
    Λ-commute x₁≈x₂ = ∷-congᵣ (begin
      ...
      ≈⟨
        Λ-cong (𝒵-cong (∘-congᵣ {γ = Comma⇒.h f} (∷-congₗ (S.sym
          (S.trans
            ∘-identityˡ
            (S.trans (∘-congᵣ (π-cong E.identity)) (S.trans π-lemma (π-cong ∘-identityʳ))))))))
      ⟩
        _
