...
module NbE.Gluing.Glue.Eval {a} (𝒰 : Set a) where
...
open import NbE.Gluing.Glue.Yoga 𝒰 using (↓₀; ↑₀; 𝓡₀; yoga₀)
open import NbE.Gluing.Glue.Cartesian 𝒰 using (⊤; _×_)
  ...
  module _ {Γ} {α₁ α₂ : Setoid.Carrier (A⇒B×A.₀ Γ)} where
    ...
    eval-commute ((tt , ((f₁′≈f₂′ , f₁≈f₂) , tt)) , x₁≈x₂) = ∷-congᵣ (begin
      ...
      ≈⟨ app-congₗ (Λ-cong (𝒵-cong (S.sym (proj₂ f₁ (cong (↑₀.η A (Γ · A)) PE.refl , PE.refl {x = ω₁ (ϵ {Γ})}))))) ⟩
        (Λ (𝓏 [ f₁′ ∘ E.₁ (ω₁ (ϵ {Γ})) ] ⦅ 𝒵 (𝔦₀.η A (Γ · A) ⟨$⟩ (↓₀.η A (Γ · A) ⟨$⟩ (↑₀.η A (Γ · A) ⟨$⟩ (R.𝓋 R.𝓏)))) ⦆)) ⦅ 𝒵 δ ⦆
      ≈⟨ app-congₗ (Λ-cong (app-congᵣ (𝒵-cong (yoga₀ PE.refl)))) ⟩
        (Λ (𝓏 [ f₁′ ∘ E.₁ (ω₁ (ϵ {Γ})) ] ⦅ 𝓏 ⦆)) ⦅ 𝒵 δ ⦆
      ≈⟨ Λβ ⟩
        𝓏 [ f₁′ ∘ E.₁ (ω₁ (ϵ {Γ})) ] ⦅ 𝓏 ⦆ [ id ∷ 𝒵 δ ]
      ≈⟨ sb-app ⟩
        (𝓏 [ f₁′ ∘ E.₁ (ω₁ (ϵ {Γ})) ] [ id ∷ 𝒵 δ ]) ⦅ 𝓏 [ id ∷ 𝒵 δ ] ⦆
      ≈⟨ app-cong₂ sb-assoc v𝓏 ⟩
        𝓏 [ (f₁′ ∘ E.₁ (ω₁ (ϵ {Γ}))) ∘ (id ∷ 𝒵 δ) ] ⦅ 𝒵 δ ⦆
      ≈⟨ app-congₗ (sb-congᵣ ∘-assoc) ⟩
        𝓏 [ f₁′ ∘ (E.₁ (ω₁ (ϵ {Γ})) ∘ (id ∷ 𝒵 δ)) ] ⦅ 𝒵 δ ⦆
      ≈⟨ app-congₗ (sb-congᵣ (∘-congᵣ (S.trans (∘-congₗ (π-cong E.identity)) (S.trans πβ ∘-identityʳ)))) ⟩
        𝓏 [ f₁′ ∘ id ] ⦅ 𝒵 δ ⦆
      ≈⟨ app-congₗ (sb-congᵣ ∘-identityʳ) ⟩
        𝓏 [ f₁′ ] ⦅ 𝒵 δ ⦆
      ≈⟨ app-congₗ (sb-congᵣ f₁′≈f₂′) ⟩
        𝓏 [ f₂′ ] ⦅ 𝒵 δ ⦆
      ≈⟨ app-congₗ (C.sym (sb-congᵣ (S.trans (∘-congᵣ E.identity) ∘-identityʳ))) ⟩
        𝓏 [ f₂′ ∘ E.₁ {A = Γ} ϵ ] ⦅ 𝒵 δ ⦆
